...
class SnippetsLocations {
  ...
  public init() throws {
    ...
    let useiCloud = MoshroomDefaults.isICloudSyncEnabled() && fm.ubiquityIdentityToken != nil

    let snippetsLocation = MoshroomPaths.localSnippetsLocationURL()!
    let icloudSnippetsLocation = MoshroomPaths.iCloudSnippetsLocationURL()
    ...
    // ".moshroom/snippets" for local
    // ".moshroom/snippets/.cached/com.github" for github
    ...
    // Locations are sorted by priority. (The old read-only GitHub snippets index was removed —
    // snippets are local / iCloud only.)
