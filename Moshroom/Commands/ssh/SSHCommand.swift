...
extension SSHCommand {
  ...
  func resolveHost() throws -> (hostName: String, host: MoshSSHHost) {
    let commandHost = try self.bkSSHHost()
    return try MoshConfig().resolveHost(alias: self.hostAlias, extending: commandHost)
  }
  ...
  func sshOptions() throws -> [String: Any] {
    ...
    for o in options {
      ...
      // The MoshSSHHost will perform further validation when generating the configuration
