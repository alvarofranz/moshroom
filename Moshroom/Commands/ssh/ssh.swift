...
@_cdecl("moshroom_ssh_main")
public func moshroom_ssh_main(argc: Int32, argv: Argv) -> Int32 {
  setvbuf(thread_stdin, nil, _IONBF, 0)
  setvbuf(thread_stdout, nil, _IONBF, 0)
  setvbuf(thread_stderr, nil, _IONBF, 0)
  
  let session = Unmanaged<MCPSession>.fromOpaque(thread_context).takeUnretainedValue()
  let cmd = MoshroomSSH(mcp: session)
  return cmd.start(argc, argv: argv.args(count: argc))
}

@objc public class MoshroomSSH: NSObject {
...
//        SSHPool.deregister(allFor: connection)
    ...
    connectionCancellable = connect.flatMap { conn -> SSHConnection in
      ...
      if cmd.startsSession {
        ...
        if let bkHost = MoshHosts.withHost(cmd.hostAlias),
  ...
  private func loadAgentForwardKeys(bkHost: MoshHosts, agent: SSHAgent) -> Bool {
    var constraints: [SSHAgentConstraint]? = nil
    let agentForwardPrompt = MoshAgentForward(UInt32(bkHost.agentForwardPrompt?.intValue ?? 0))

    if agentForwardPrompt == MoshAgentForwardConfirm {
      constraints = [SSHAgentUserPrompt()]
    } else if agentForwardPrompt == MoshAgentForwardYes {
      constraints = []
    } else {
      return false
    }
    ...
    if constraints != nil {
      let _allIdentities = MoshPubKey.all()
