...
struct MoshroomCopyCommand: ParsableCommand {
...
enum MoshroomFilesProtocols: String {
  case local = "local"
  case scp = "scp"
  case sftp = "sftp"
}
...
class FileLocationPath {
  ...
  var proto: MoshroomFilesProtocols?
  ...
  init(_ path: String) throws {
    ...
    default:
      ...
      self.proto = MoshroomFilesProtocols(rawValue: proto)
    }
  }
}


public class MoshroomCopy: NSObject {
  ...
  public func start(_ argc: Int32, argv: [String]) -> Int32 {
    ...
    do {
      command = try MoshroomCopyCommand.parse(Array(argv[1...]))
    } catch {
      let message = MoshroomCopyCommand.message(for: error)
  ...
  func localTranslator(to path: String) -> AnyPublisher<Translator, Error> {
    return .just(MoshroomFiles.Local())
  }

  func remoteTranslator(toFilePath filePath: String, atHost hostPath: String, using proto: MoshroomFilesProtocols, isSource: Bool = true) -> AnyPublisher<Translator, Error> {
    ...
    return SSHClient.dial(hostNameResolved, with: config, withProxy: MoshroomSSH.executeProxyCommand)
