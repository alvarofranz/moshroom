...
private var _showReplHints: Bool = {
  guard
    let value = getenv("MOSHROOM_REPL_HINTS")
  else {
    return true
  }
...
//  //    @"unsetenv": @"", // TODO
  ...
  static func _for(cursor: Int, str: String, n: Int) -> (kind: Kind, start: Int, pos: Int, len: Int, result: String, hint: String) {
    ...
    var prefix = ""
    if kind == .moshroomHost, let atIndex = input.firstIndex(of: "@") {
  ...
  static func _complete(kind: Kind, input: String) -> [String] {
    ...
    case .moshroomHost: src = _allMoshroomHosts();
    default: break
    }
  ...
  private static func _allHosts() -> [String] {
    let moshroomHosts = _allMoshroomHosts()
    let knownHosts = _allKnownHosts()
    
    let set = Set(moshroomHosts).union(knownHosts)
    return Array(set).sorted()
  }
  
  private static func _allMoshroomHosts() -> [String] {
    let hosts: Set<String> = Set(
      (MoshHosts.all() ?? [])
        .compactMap({$0 as? MoshHosts})
        .compactMap({$0.host})
    )
  ...
  private static func _allKnownHosts() -> [String] {
    guard let hostsFile = try? String(contentsOfFile: MoshroomPaths.knownHostsFile(), encoding: .utf8)
    else {
      return []
    }
  ...
  private static func _allPaths(prefix: String, skipFiles: Bool) -> [String] {
    var pref = prefix
    let home = MoshroomPaths.homePath()!
    var cleanup = false
    if pref.hasPrefix("~") {
