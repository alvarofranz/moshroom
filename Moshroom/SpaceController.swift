...
class SpaceController: UIViewController {
  ...
  public override func viewDidLoad() {
    ...
    // Moshroom: install the Moshxplore remote file explorer (opened from its top-bar button).
    Moshxplore.install(in: self)
...
extension SpaceController {
  ...
  func moshroomTabs() -> [MoshroomTabInfo] {
    _viewportsKeys.map { key in
      ...
      // The program's own terminal title (what claude / vim / ssh set via the OSC title
      // sequence) unless a custom name was forced. Unloaded tabs have no live title, so "shell".
      let osc = (term.termView.title ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
      let name = !custom.isEmpty ? custom : (osc.isEmpty ? "shell" : osc)
