...
@interface TermView () <WKScriptMessageHandler, WKUIDelegate, UIGestureRecognizerDelegate, UIEditMenuInteractionDelegate>
@end
...
//  configuration.limitsNavigationsToAppBoundDomains = YES;
  ...
  _webView.opaque = NO;
  // No text caret in the terminal — you type in Moshkitor, not here. The iOS contentEditable caret
  // follows the view's tintColor (CSS caret-color alone doesn't reliably reach it inside hterm), so
  // clear the tint. Moshkitor is a separate view and keeps its own accent-coloured caret.
  _webView.tintColor = [UIColor clearColor];
