...
- (id)init
{
  ...
  if (self) {
    ...
    _queue = dispatch_queue_create("moshroom.TermDevice", NULL);
...
- (void)write:(NSString *)input
{
  ...
  // On the Moshroom prompt, atm this is a special mode as it doesn't have a "readline" per-se.
...
- (void)setAutoCR:(BOOL)autoCR {
  if (autoCR) {
    fprintf(_stream.out, "\x1b]1337;MoshroomAutoCR=1\x07");
  } else {
    fprintf(_stream.out, "\x1b]1337;MoshroomAutoCR=0\x07");
  }
...
- (void)prompt:(NSString *)prompt secure:(BOOL)secure shell:(BOOL)shell {
  ...
  NSString *cmd = [NSString stringWithFormat: @"\x1b]1337;MoshroomPrompt=%@\x07", [data base64EncodedStringWithOptions:kNilOptions]];
