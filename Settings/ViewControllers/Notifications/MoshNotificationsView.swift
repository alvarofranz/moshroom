...
struct MoshNotificationsView: View {
...
fileprivate enum MoshNotifications: CaseIterable {
...
struct NotifyNotificationsView: View {
  var body: some View {
    VStack(alignment: .leading) {
      Text("Moshroom supports standard OSC sequences & iTerm2 growl notifications. Some OSC sequences might not be supported in Mosh.\n\nExamples (tap to copy & use them on a SSH connection):")
      
      ForEach(MoshNotifications.allCases, id: \.self) { notification in
...
class NotificationConfig: ObservableObject {
  @Published var playSoundOnActiveShell: Bool {
    didSet {
      MoshroomDefaults.setPlaySoundOnBell(playSoundOnActiveShell)
    }
  }
  ...
  @Published var notificationOnBackgroundShell: Bool {
    didSet {
      _askForNotificationPermissions(completion: { granted in
        ...
        MoshroomDefaults.setNotificationOnBellUnfocused(self.notificationOnBackgroundShell)
      })
    }
  }
  ...
  @Published var useHapticFeedback: Bool {
    didSet {
      MoshroomDefaults.setHapticFeedbackOnBellOff(!useHapticFeedback)
    }
  }
  ...
  @Published var notifyNotifications: Bool {
    didSet {
      ...
      _askForNotificationPermissions(completion: { granted in
        ...
        MoshroomDefaults.setOscNotifications(self.notifyNotifications)
      })
    }
  }
  ...
  init() {
    playSoundOnActiveShell = MoshroomDefaults.isPlaySoundOnBellOn()
    notificationOnBackgroundShell = MoshroomDefaults.isNotificationOnBellUnfocusedOn()
    useHapticFeedback = !MoshroomDefaults.hapticFeedbackOnBellOff()
    notifyNotifications = MoshroomDefaults.isOscNotificationsOn()
  }
}
