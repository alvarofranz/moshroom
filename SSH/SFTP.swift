...
public class SFTPTranslator: MoshroomFiles.Translator {
  ...
  public func create(name: String, mode: mode_t = S_IRWXU) -> AnyPublisher<MoshroomFiles.File, Error> {
    if fileType != .typeDirectory {
      return .fail(error: FileError(title: "Not a directory.", in: session))
    }
...
public class SFTPFile : MoshroomFiles.File {
  var file: sftp_file?
  let sftpClient: SFTPClient
  var sftp: sftp_session { sftpClient.sftp }
  var channel: ssh_channel { sftpClient.channel }
  ...
  // libssh 0.11 replaced the (patched) sftp_async_write id model with the upstream sftp_aio
  // handle. Each in-flight write is its opaque handle + the byte count it actually queued
  // (begin_write may queue fewer bytes than requested), so accounting sums real bytes, not blocks.
  var inflightWrites: [(aio: OpaquePointer?, len: Int)] = []
...
extension SFTPFile: MoshroomFiles.Writer {
  ...
  public func write(_ buf: DispatchData, max length: Int) -> AnyPublisher<Int, Error> {
    ...
    func writeLoop(_ w: DispatchData, _ wn: DispatchData) {
      ...
      if inflightWrites.count > 0 {
        // Check scheduled writes: checkWrites acks completed aios, removes them from the
        // in-flight list, and returns the real number of bytes acknowledged this pass.
        do {
          writtenBytes = try self.checkWrites()
          if writtenBytes > 0 {
            written = written.subdata(in: writtenBytes..<written.count)
          }
          if w.count == 0 && inflightWrites.isEmpty {
            isFinished = true
          }
        } catch {
      ...
      while inflightWrites.count < self.maxConcurrentOps && write.count > 0 {
        ...
        var aio: OpaquePointer? = nil
        let queued = write.withUnsafeBytes { bytes -> Int in
          return sftp_aio_begin_write(self.file, bytes, length, &aio)
        }

        if queued < 0 {  // SSH_ERROR
          pb.send(completion: .failure(FileError(title: "Could not pre-alloc write request", in: session)))
          return
        }

        inflightWrites.append((aio, queued))
        // Advance by the bytes actually queued (may be < length).
        write = write.subdata(in: queued..<write.count)
      }
  ...
  // Acknowledge as many in-flight sftp_aio writes as are ready (non-blocking), stopping at the
  // first not-yet-acked one (SSH_AGAIN). Returns the total bytes acknowledged this pass and drops
  // the completed handles from `inflightWrites`. sftp_aio_wait_write frees the handle on success.
  func checkWrites() throws -> Int {
    var ackedBytes = 0
    var completed = 0

    for (aio, len) in inflightWrites {
      var handle = aio
      self.log.message("sftp_aio_wait_write sent", SSH_LOG_DEBUG)
      let rc = sftp_aio_wait_write(&handle)
    ...
    if completed > 0 {
      inflightWrites = Array(inflightWrites[completed...])
    }
