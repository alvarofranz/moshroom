...
public class SSHClient {
  ...
  private init(to host: String, with opts: SSHClientConfig, proxyCb: ExecProxyCommandCallback?) throws {
    ...
    if let sshDir = options.sshDirectory {
      try _setSessionOption(SSH_OPTIONS_SSH_DIR, sshDir)
      // libssh 0.11+ no longer derives the per-user known_hosts path from SSH_DIR alone at
      // update time — `ssh_session_update_known_hosts` fails ("Error updating known_hosts file")
      // unless the file path is set explicitly. Point it at <sshDir>/known_hosts.
      let knownHosts = (sshDir as NSString).appendingPathComponent("known_hosts")
      try _setSessionOption(SSH_OPTIONS_KNOWNHOSTS, knownHosts)
    }
  ...
  public func requestReverseForward(bindTo address: String?, port: Int32) -> AnyPublisher<PassthroughSubject<Stream, Error>, Error> {
    ...
    // libssh 0.12 renamed this callback to *_forwarded_tcpip and widened its signature
    // (destination + originator address/port). We key reverse forwards by destination port, so
    // that's the field we read; the rest is unused.
    let cb: ssh_channel_open_request_forwarded_tcpip_callback = { (session, _destAddr, destPort, _origAddr, _origPort, userdata) in
      let ctxt = Unmanaged<SSHClient>.fromOpaque(userdata!).takeUnretainedValue()

      ctxt.log.message("REVERSE Forward callback", SSH_LOG_INFO)

      let port = Int32(destPort)
    ...
    return connection()
      .tryOperation { session -> Int32 in
        ...
        if self.callbacks.channel_open_request_forwarded_tcpip_function == nil {
