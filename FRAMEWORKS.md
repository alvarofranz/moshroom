...
Everything Moshroom links, in one place. **100% self-hosted, zero Blink, zero third-party hosting.**
...
There is **no build-from-source step** and **no Blink tooling** — the old `xcfs` framework-builder
(`build-project` + `FMake` + the `BlinkCore` project) was deleted.
...
- **swift-argument-parser** — Apple's own, pinned `.exact("0.5.0")`
  ...
  `ssh`/`mosh`/`scp` command parsers. (Was a Blink fork; swapped to canonical Apple upstream.)
- **SSHConfig** — vendored at [`xcfs/SSHConfig/`](xcfs/SSHConfig) (was `blinksh/SSHConfig` 0.0.5,
  a small dependency-free SSH-config parser). In-tree so the app is fully self-contained.
