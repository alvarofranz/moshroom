                         ...
                         void **abstract)
{
  ...
  for (int i = 0; i < num_prompts; i++) {
    ...
    NSString *text = [[NSString alloc] initWithBytes:prompt.text length:prompt.length encoding:NSUTF8StringEncoding];
...
- (int)main:(int)argc argv:(char **)argv
{
  ...
  if (!_options.user) {
    ...
    _options.user = [[MoshroomDefaults defaultUserName] UTF8String];
  }
...
- (void)processHostSettings
{
  ...
  if (!(host = [MoshHosts withHost:[NSString stringWithUTF8String:_options.hostname]])) {
    return;
  }
...
- (void)load_identity_files
{
  ...
  if (_options.identity_file) {
    if ((pk = [MoshPubKey withID:[NSString stringWithUTF8String:_options.identity_file]]) != nil) {
      [_identities addObject:pk];
    }
  }
  
  if ((pk = [MoshPubKey withID:@"id_rsa"]) != nil) {
    [_identities addObject:pk];
  }
}
...
- (int)ssh_login_publickey:(const char *)user
{
  ...
  for (MoshPubKey *pk in _identities) {
...
//  NSURL *khURL = [dd URLByAppendingPathComponent:@"known_hosts"];
  const char *khFilePath = [MoshroomPaths knownHostsFile].UTF8String;// [khURL.path UTF8String];
