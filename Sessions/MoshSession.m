...
+ (void)initialize
{
  predictionModeStrings = @{
    @(MoshMoshPredictionAdaptive): @"adaptive",
    @(MoshMoshPredictionAlways): @"always",
    @(MoshMoshPredictionNever): @"never",
    @(MoshMoshPredictionExperimental): @"experimental"
  };
  
  experimentalIPStrings = @{
    @(MoshMoshExperimentalIPNone): @"default",
    @(MoshMoshExperimentalIPLocal): @"local",
    @(MoshMoshExperimentalIPRemote): @"remote"
  };
}
...
- (int)initParamaters:(int)argc argv:(char **)argv
{
  ...
  MoshHosts *hostCfg;
  if ([chunks count] != 2) {
    hostCfg = [MoshHosts withHost:userhost];
  } else {
    hostCfg = [MoshHosts withHost:chunks[1]];
  }
...
- (void)processMoshSettings:(MoshHosts *)host
{
                       ...
                       error:(NSError **)error
{
  ...
  if (term_r == NULL) {
    *error = [NSError errorWithDomain:@"moshroom.mosh.ssh" code:0 userInfo:@{ NSLocalizedDescriptionKey : @"SSH session exited with error. Try SSH to the host first." }];
    return;
  }
  ...
  while ((n = getline(&buf, &buf_sz, term_r)) >= 0) {
    ...
    if (ipFormat != NULL) {
      ...
      if (resolvedAddress == NULL) {
        *error = [NSError errorWithDomain:@"moshroom.mosh.ssh" code:0 userInfo:@{ NSLocalizedDescriptionKey : @"Could not resolve address locally for host." }];
        return;
      }
  ...
  if (!match) {
    *error = [NSError errorWithDomain:@"moshroom.mosh.ssh" code:0 userInfo:@{ NSLocalizedDescriptionKey : @"Could not start mosh-server." }];
    return;
  }
  
  if (!self.sessionParams.ip) {
    *error = [NSError errorWithDomain:@"moshroom.mosh.ssh" code:0 userInfo:@{ NSLocalizedDescriptionKey : @"Incorrect mosh-server startup sequence." }];
    return;
  }
...
- (void)setConnParamsWithSsh2:(NSString *)ssh userHost:(NSString *)userHost port:(NSString *)port identity:(NSString *)identity moshCommand:(NSString *)command error:(NSError **)error
{
  ...
  if (!self.sessionParams.ip) {
    *error = [NSError errorWithDomain:@"moshroom.mosh.ssh" code:0 userInfo:@{ NSLocalizedDescriptionKey : @"Did not find remote IP address" }];
    return;
  }
