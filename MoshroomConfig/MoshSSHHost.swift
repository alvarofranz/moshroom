...
public struct MoshSSHHost {
  ...
  public func merge(_ host: MoshSSHHost) throws -> MoshSSHHost {
    var configDict = content
    configDict.mergeWithSSHConfigRules(host.content)
    return try MoshSSHHost(content: configDict)
  }
  ...
  public func sshClientConfig(authMethods: [SSH.AuthMethod]?,
    ...
    SSHClientConfig(
      ...
      sshDirectory: MoshroomPaths.ssh()!,
      logger: logger,
...
extension Bool: SSHValue {
  fileprivate init(castSSHValue val: String) throws {
    ...
    case "no":
      self.init(false)
    default:
      throw MoshSSHHost.ValidationError(message: "Value must be yes/no")
    }
  }
}
...
extension Int: SSHValue {
  fileprivate init(castSSHValue val: String) throws {
    guard let _ = Int(val) else {
      throw MoshSSHHost.ValidationError(message: "Value must be a number")
    }
    self.init(val)!
  }
}
...
extension UInt: SSHValue {
  fileprivate init(castSSHValue val: String) throws {
    guard let _ = UInt(val) else {
      throw MoshSSHHost.ValidationError(message: "Value must be a positive integer.")
    }
    self.init(val)!
  }
}
...
extension SSHLogLevel: SSHValue {
  fileprivate init(castSSHValue val: String) throws {
    guard let _ = SSHLogLevel(val) else {
      throw MoshSSHHost.ValidationError(message: "Value must be QUIET, etc...")
    }
    self.init(val)!
  }
}
...
public enum TTYBool: String, SSHValue {
  ...
  fileprivate init(castSSHValue val: String) throws {
    guard let _ = TTYBool(rawValue: val) else {
      throw MoshSSHHost.ValidationError(message: "Value must be auto, force, yes or no")
    }
...
public enum ControlMasterOption: String, SSHValue {
  ...
  fileprivate init(castSSHValue val: String) throws {
    guard let _ = ControlMasterOption(rawValue: val) else {
      throw MoshSSHHost.ValidationError(message: "Value must be auto, ask, autoask, yes or no")
    }
    self.init(rawValue: val)!
  }
}
...
public struct PortForwardInfo: Equatable, SSHValue {
  ...
  public init(_ info: String) throws {
    ...
    guard let match = regex.firstMatch(in: info,
      ...
      throw MoshSSHHost.ValidationError(message: "Missing <localport>:<bind_address>:<remoteport> for port forwarding.")
    }
    guard let r = Range(match.range(withName: "localPort"), in: info),
          let localPort = UInt16(info[r])
    else {
      throw MoshSSHHost.ValidationError(message: "Invalid local port.")
    }
    ...
    guard let r = Range(match.range(withName: "remotePort"), in: info),
      ...
      throw MoshSSHHost.ValidationError(message: "Invalid remote port.")
    }
    self.remotePort = remotePort

    guard let r = Range(match.range(withName: "bindAddress"), in: info)
    else {
      throw MoshSSHHost.ValidationError(message: "Invalid bind address.")
    }
...
public struct OptionalBindAddressInfo: Equatable, SSHValue {
  ...
  public init(_ info: String) throws {
    ...
    guard let match = regex.firstMatch(in: info,
      ...
      throw MoshSSHHost.ValidationError(message: "Missing <bind_address>:<remoteport>.")
    }
...
public struct BytesNumber: SSHValue {
  ...
  fileprivate init(castSSHValue val: String) throws {
    guard let lastChar = val.last else {
      throw MoshSSHHost.ValidationError(message: "Missing string")
    }
    ...
    case "K":
      number = number * 1024
    default:
      throw MoshSSHHost.ValidationError(message: "Invalid bytes number.")
    }
